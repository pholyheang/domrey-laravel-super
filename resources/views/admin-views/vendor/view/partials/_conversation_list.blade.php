@forelse($conversations as $conv)
    ...
    @if (isset($user) && $conv->last_message)
        ...
        <div class="chat-user-info d-flex border-bottom p-3 align-items-center customer-list view-dm-conv {{ $unchecked != 0 ? 'conv-active' : '' }}"
            data-url="{{ route('admin.users.delivery-man.message-view', ['conversation_id' => $conv->id, 'user_id' => $user->id]) }}"
            data-active-id="customer-{{ $user->id }}" data-conv-id="{{ $conv->id }}"
...
@empty
    <div class="h-98p d-center">
        <div class="empty--data">
            <img width="64" class="mb-2" src="{{ asset('/assets/admin/img/no-vendor-list.svg') }}"
                alt="public">
            <p class="fs-16 mb-20">
