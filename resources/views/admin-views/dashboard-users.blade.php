...
<div class="content container-fluid">
    @if(auth('admin')->user()->role_id == 1)
        ...
        <div class="row g-2 pb-4 mb-0">
            ...
            <div class="col-sm-6 col-lg-{{ $card_col_class }}">
            ...
            <div class="col-sm-6 col-lg-{{ $card_col_class }}">
                <a href="{{ route('admin.users.rider.list', ['zone_id' => $params['zone_id'] ?? null]) }}">
                    <div class="__user-dashboard-card" style="--theme-clr:#006AB4">

                        <div class="__user-dashboard-card-thumbs">
                            @php($total_riders = $rider_data['total_riders'] ?? 0)
                            <div class="more-icon">
                                +{{$total_riders >= 4 ? $total_riders - 2 : $total_riders}}
                            </div>
            ...
            <div class="row g-2">
                <div class="col-lg-8">
                    <div class="row gap__10">
                        <div class="col-md-3 col-sm-6">
                            <a
                                href="{{ route('admin.users.rider.list', ['zone_id' => $params['zone_id'] ?? null, 'filter' => 'active']) }}">
                                <div class="__customer-statistics-card h-100">
                                    <div class="title">
                                        <img src="{{asset('/assets/admin/img/new-img/deliveryman/active.svg')}}"
                                            alt="new-img">
                        ...
                        <div class="col-md-3 col-sm-6">
                            <a
                                href="{{ route('admin.users.rider.list', ['zone_id' => $params['zone_id'] ?? null, 'filter' => 'new']) }}">
                                <div class="__customer-statistics-card h-100" style="--clr:#006AB4">
                                    <div class="title">
                                        <img src="{{asset('/assets/admin/img/new-img/deliveryman/newly.svg')}}"
                                            alt="new-img">
                        ...
                        <div class="col-md-3 col-sm-6">
                            <a
                                href="{{ route('admin.users.rider.list', ['zone_id' => $params['zone_id'] ?? null, 'filter' => 'inactive']) }}">
                                <div class="__customer-statistics-card h-100" style="--clr:#FF5A54">
                                    <div class="title">
                                        <img src="{{asset('/assets/admin/img/new-img/deliveryman/in-active.svg')}}"
                                            alt="new-img">
                        ...
                        <div class="col-md-3 col-sm-6">
                            <a
                                href="{{ route('admin.users.rider.list', ['zone_id' => $params['zone_id'] ?? null, 'filter' => 'blocked']) }}">
                                <div class="__customer-statistics-card h-100" style="--clr:#FF5A54">
                                    <div class="title">
                                        <img src="{{asset('/assets/admin/img/new-img/customer/blocked.svg')}}"
                                            alt="new-img">
                    ...
                    <div class="__map-wrapper-2 mt-3">
                        ...
                        <div class="map-warper map-wrapper-2 rounded">
                            <div id="rider-map-canvas" class="rounded"></div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="card h-100" id="top-deliveryman-view">
                        @include('admin-views.partials._top-riders', ['top_riders' => $rider_data['top_riders']])
                    </div>
                </div>
            </div>
        @endif
...
<script>
    ...
    function initialize() {
        ...
        if (deliveryMan.length > 0) {
            map.fitBounds(dmbounds);
            let dmZoomListener = google.maps.event.addListener(map, 'idle', function() {
                if (map.getZoom() > 16) map.setZoom(16);
                google.maps.event.removeListener(dmZoomListener);
            });
        }
        @if(addon_published_status('RideShare'))
        initializeRiderMap();
    ...
    function initializeRiderMap() {
        ...
        riderData.forEach(rider => {
            if (rider.lat) {
                const point = new google.maps.LatLng(rider.lat, rider.lng);
                ...
                google.maps.event.addListener(marker, 'click', function () {
                    infowindow.setContent(`
                <div style='float:left'>
                    <img style='max-height:40px;wide:auto;' src='${rider.image_link}'>
                </div>
                <div style='float:right; padding: 10px;'>
                    <b>${rider.name}</b><br/>
                    ${rider.location}<br/>
                    Assigned Trip: ${rider.assigned_order_count}
                </div>`);
                    infowindow.open(riderMap, marker);
                });
            }
        });
        if (riderData.length > 0) {
            riderMap.fitBounds(riderBounds);
            let riderZoomListener = google.maps.event.addListener(riderMap, 'idle', function() {
                if (riderMap.getZoom() > 16) riderMap.setZoom(16);
                google.maps.event.removeListener(riderZoomListener);
            });
        }
    }
    @endif
    ...
    $('#rider-search-form').on('submit', function (e) {
        ...
        $.post({
            url: '{{ route('admin.users.rider.active-search') }}',
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
            success: function (data) {
                ...
                if (data.dm) {
                    riderData.forEach(item => {
                        ...
                        const isRiderActive = data.dm.some(ddm => ddm.id === item.id);
                        if (isRiderActive) {
                            itemCount++
                        }
                        ...
                        const newIconContent = document.createElement("img");
                        newIconContent.src = icon;
                        newIconContent.alt = isRiderActive ? "Active" : "Inactive";
                        newIconContent.style.width = '100%';
                        newIconContent.style.height = '100%';
                        newIconContent.style.borderRadius = '50%';

                        const markerToUpdate = riderMarkers[item.id];
                        if (markerToUpdate) {
                            markerToUpdate.content = newIconContent;
                            ...
                            riderMap.panTo(markerToUpdate.position);
                            riderMap.setZoom(16);
                            let riderViewContent = `
                <div style='float:left'>
                    <img style='max-height:40px;wide:auto;'  src='${item.image_link}'>
                </div>
                <div style='float:right; padding: 10px;'>
                    <b>${item.name}</b><br/>
                    ${item.location}<br/>
                    Assigned Trip: ${item.assigned_order_count}
                </div>`

                            if (isRiderActive && itemCount == 1) {
                                infowindow.setContent(riderViewContent);
                                infowindow.open(riderMap, markerToUpdate);
                            } else {
                                google.maps.event.addListener(markerToUpdate, 'click', function () {
                                    infowindow.setContent(riderViewContent);
                                    infowindow.open(riderMap, markerToUpdate);
