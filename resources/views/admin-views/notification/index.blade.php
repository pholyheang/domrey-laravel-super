...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="fs-12 px-3 py-2 rounded bg-warning bg-opacity-10 text-title d-flex gap-2 align-items-baseline mb-20">
            ...
            <span class="fs-12 mb-0">
                ...
                <a target="_blank" href="{{ route('admin.business-settings.fcm-config') }}"
                    class="font-semibold text-info">{{ translate('messages.Firebase Configuration') }} </a>
            ...
            <form action="{{ route('admin.notification.store') }}" method="post" enctype="multipart/form-data"
                        ...
                        <div class="__bg-F8F9FC-card h-100">
                            <div class="row g-3">
                                <div class="col-sm-6">
                                    <div class="form-group mb-0">
                                        <label class="input-label text-capitalize"
                                            for="exampleFormControlInput1">{{ translate('messages.zones') }}
                                            <span class="text-danger">*</span>
                                        </label>
                                        <select name="zone" id="zone" class="form-control js-select2-custom"
                                            data-search-placeholder="{{ translate('messages.search_zone') }}">
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group mb-0">
                                        <label class="input-label text-capitalize"
                                            for="tergat">{{ translate('messages.Targeted user') }} <span
                                    ...
                                    <div class="form-group mb-0">
                                        <label class="input-label text-capitalize d-flex gap-1 align-items-center"
                                            ...
                                            <span data-toggle="tooltip"
                                                data-title="{{ translate('messages.enter_notification_title') }}">
                                                <i class="tio-info text-light-gray"></i>
                                            </span>
                                            <span class="text-danger">*</span>
                                        </label>
                                        <textarea name="notification_title" class="form-control" maxlength="100" rows="1"
                                            placeholder="{{ translate('messages.Type_Title') }}" required></textarea>
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group mb-0">
                                        <label class="input-label text-capitalize d-flex gap-1 align-items-center"
                                            ...
                                            <span data-toggle="tooltip"
                                                data-title="{{ translate('messages.enter_notification_description') }}">
                                                <i class="tio-info text-light-gray"></i>
                                            </span><span class="text-danger">*</span>
                                        </label>
                                        <textarea name="description" class="form-control" maxlength="200" rows="1"
                ...
                <div class="btn--container justify-content-end">
                    ...
                    <button type="submit" id="submit"
                        class="btn btn--primary">{{ translate('messages.Save_&_Send') }}</button>
                </div>
            </form>
        </div>
        ...
        <div class="card card-body">
            <div class="d-flex flex-wrap justify-content-lg-end align-items-center gap-3 mb-20">
                <h4 class="mb-0 flex-grow-1">{{ translate('Notification History') }}<span
                        class="badge badge-soft-dark ml-2">{{ $notifications->total() }}</span></h4>
                ...
                <select name="target" class="form-control custom-select max-w-200px min-w-100-mobile" id="filter_form"
                    ...
                    <option value="all" {{ $target == 'all' ? 'selected' : '' }}>{{ translate('messages.all') }}
                    </option>
                ...
                <div class="hs-unfold mr-2">
                    <a class="js-hs-unfold-invoker btn btn-sm btn-white dropdown-toggle min-height-40 h--45px"
                        href="javascript:;"
                    ...
                    <tbody>
                        @foreach ($notifications as $key => $notification)
                            <tr>
                                ...
                                <td>
                                    <span data-toggle="tooltip" data-title="{{ $notification['title'] }}"
                                ...
                                <td>
                                    ...
                                    <span class="max-w-280 line--limit-2" data-toggle="tooltip"
                                ...
                                <td>
                                    <div class="btn--container justify-content-center">
                                        <button type="button" class="btn action-btn btn--primary btn-outline-primary"
                                            data-toggle="modal" data-target="#notification-view-modal"
                                            title="{{ translate('messages.view_notification') }}"
                                        ...
                                        <button type="button"
                                            ...
                                            title="{{ translate('messages.edit_notification') }}"
    ...
    <div id="notification-update-offcanvas" class="custom-offcanvas d-flex flex-column justify-content-between">
        <form action="" method="post" enctype="multipart/form-data" id="update-notification-form">
            ...
            <div>
                <div
                    class="custom-offcanvas-header bg--secondary d-flex justify-content-end align-items-center  gap-3 px-3 py-3">
                ...
                <div class="custom-offcanvas-body custom-offcanvas-body-100  p-20">
                    <div class="mb-9">
                        ...
                        <div class="__bg-F8F9FC-card mb-20">
                            ...
                            <div class="form-group mb-3">
                                <label class="input-label text-capitalize"
                                    for="exampleFormControlInput1">{{ translate('messages.zones') }} <span
                                        class="text-danger">*</span></label>
                                <select name="zone" id="zone_u"
                                    data-search-placeholder="{{ translate('messages.search_zone') }}"
                                    class="form-control custom-select js-select2-custom">
                            ...
                            <div class="form-group mb-0">
                                <label class="input-label text-capitalize"
                                    for="tergat">{{ translate('messages.Targeted_user') }} <span
                                        class="text-danger">*</span></label>
                ...
                <div
                    class="offcanvas-footer d-flex gap-3 justify-content-center align-items-center bg-white bottom-0 mt-auto p-3">
                    <button type="button" id="update_reset_btn"
                        class="btn btn--reset w-100">{{ translate('reset') }}</button>
    ...
    <script>
      ...
      $('.edit-btn').on('click', function() {
            ...
            originalNotificationData = {
                ...
                zone: $(this).data('zone'),
                tergat: $(this).data('tergat')
            };

            $('#notification_title_u').val(originalNotificationData.title).trigger('input');
            $('#description_u').val(originalNotificationData.description).trigger('input');
            $('#zone_u').val(originalNotificationData.zone ? originalNotificationData.zone : 'all').trigger('change');
            $('#tergat_u').val(originalNotificationData.tergat);

            var $container = $('#image-input-u').closest('.upload-file_custom');
            var $overlay = $container.find('.overlay');

            if (originalNotificationData.image) {
                $container.find('.upload-file-img').attr('src', originalNotificationData.image).show();
            ...
            action = action.replace('temp_id', originalNotificationData.id);
            $('#update-notification-form').attr('action', action);
        });

        $('#update_reset_btn').click(function() {
